...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals)!=len(vals):
            raise SchemeError('length not equal')
        while len(formals)!=0:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    if len(expressions)==2 and not expressions.first:
        return okay
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        val=bindings.first.second.first
        formal=Pair(bindings.first.first, formal)
        value=Pair(scheme_eval(val, env), value)
        checked.append(bindings.first.first)
        bindings=bindings.second
    check_formals(formal)
    return env.make_child_frame(formal, value)
    # END Question 16
...
def check_formals(formals):
    ...
    checked=[]
    while len(formals)!=0:
        if not scheme_symbolp(formals.first) or not scheme_listp(formals):
            raise SchemeError
        if formals.first in checked:
            raise SchemeError
        else:
            checked.append(formals.first)
