...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([place for place in centroids], key=lambda element: distance(location,element))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    (I use r to represent restaurant same as instrutor did in provided code)
    """
    # BEGIN Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([(pair[0]-mean_x)*(pair[1]-mean_y) for pair in zip(xs,ys)])
    b=s_xy/s_xx
    a=mean_y-b*mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    (I seperated my code to two lines so return statement doesnt look too long)
    """
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 8
    list_of_tuple=[find_predictor(user,reviewed,feature) for feature in feature_fns]
    return max(list_of_tuple, key=lambda item: item[1])[0]
    # END Question 8
