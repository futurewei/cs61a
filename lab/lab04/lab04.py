...
def closer_city(lat, lon, city1, city2):
    ...
    distance1= distance(temp, city1)
    distance2= distance(temp, city2)
...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    return  [create_row(columns) for x in range(0,rows)]
...
def replace_elem(lst, index, elem):
    ...
    "*** YOUR CODE HERE ***"
    return [elem if index==lst.index(x) else x for x in lst]
...
def put_piece(board, max_rows, column, player):
    ...
    k=1
    while k<=max_rows:
        if get_piece(board, k-1, column)=='-':
            row=k-1
            new_list=replace_elem(board[row],column,player)
            new_board=replace_elem(board,row,new_list)
...
def print_board(board, max_rows, max_cols):
    ...
    for x in range(0,rows):
        print(get_piece(board,x,0))
        for y in range(1, columns):
            print( get_piece(board, x, y))

def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    """Returns True if the given player has a horizontal win in the given row,
    and False otherwise.
