...
def make_pytunes(username):
    ...
    """
    "*** YOUR CODE HERE ***"
    return tree(username, [tree('pop',[tree('justin bieber',
                                            [tree('single',
                                                  [tree('what do you mean?')])]),
                                       tree('2015 pop mashup')]),
                           tree('trance',[tree('darude',
                                               [tree('sandstorm')])])])
# Q5
def num_songs(t):
...
def find(t, target):
    ...
    if root(t)==target:
        ...
        for branch in branches(t):
            val= find(branch,target) or False
            if val!=True:
                continue
            else:
                return True
...
def add_song(t, song, category):
    ...
    "*** YOUR CODE HERE ***"
    if root(t)==category:
        return tree(root(t), branhces(t)+[tree(song)])
    else:
        remain=[]
        for b in branhces(t):
            remain+=[add_song(b,song,category)]
        return tree(root(t), remain)
# Q8
def delete(t, target):
