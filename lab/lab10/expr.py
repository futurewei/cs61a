...
class LambdaExpr(Expr):
    ...
    [Name('x'), Name('y')].
    一个储存函数信息的自定义lambda func，把变量，运算方法存着。知道你喊eval，才开始计算。
    """
...
class CallExpr(Expr):
    ...
    功能：计算  不是lambda func，不算func，就是你在terminal打出 (add 2 3)=5之类的一些基本计算
    """
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        op=[x.eval(env) if type(x)!=Number else x for x in self.operands]
        return env[self.operator.string].apply(op)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        ****
        ...
        往env里添加新element {'A': 3, 'B': 4} 形参A:3 形参B:4
        然后callexp进行计算时候，会调用add 在 3, 4上。 都是去global environement 那个dict找的
        这个dict里存着所有加减乘除方法，和所有变量。
        ...
        lis=[x for x in self.body.operands if type(x) is Literal]+[a for a in arguments]
        return env[self.body.operator.string].apply([a.eval(env) if type(a)!=Number else a for a in lis ])
